// <copyright file="CombinedUserContextProvider.cs" company="Shohoz">
...
namespace Platform.Infrastructure.Host.WebApi
{
    ...
    public class CombinedUserContextProvider : IUserContextProvider
    {
        ...
        private UserContext GetHttpUserContext()
        {
            ...
            {
                Email = claimsIdentity.HasClaim(claim => claim.Type.Equals(Common.Security.ClaimTypes.Email)) ? claimsIdentity.Claims.First(c => c.Type == Common.Security.ClaimTypes.Email).Value.ToLower() : "info@com",
                LanguageCode = "en-US",
